...
inline void Transform(Vector4& vector, const Matrix4x4f& matrix)
{
    vector = matrix * vector;
}

inline void Transform(Vector3& vector, const Matrix4x4f& matrix)
{
    vector = Vector3(matrix * Vector4(vector, 1.0f));
}

inline void Translate(Matrix4x4f& matrix, const Vector3& pos) {
...
inline void Scale(Matrix4x4f& matrix, const Vector3& scale)
{
	matrix[0][0] *= scale.x;
	matrix[1][1] *= scale.y;
	matrix[2][2] *= scale.z;
...
inline void Rotate(Matrix4x4f& matrix, const float& tangle, Vector3 axis)
{
	Normalize(axis);
...
inline void Rotation(Matrix4x4f& matrix, const float& tangle, Vector3&& axis) {
...
inline void LookAt(Matrix4x4f& matrix, const Vector3& pos, const Vector3& goal, const Vector3& WorldUp) {

	Matrix4x4f translate = Matrix4x4f::GetIdentityMatrix();
	Translate(translate, -pos);

	Vector3 front = Unit(goal - pos);
