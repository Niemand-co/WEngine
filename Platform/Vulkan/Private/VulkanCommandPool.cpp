...
namespace Vulkan
{
	...
	RHICommandBuffer* VulkanCommandPool::GetCommandBuffer(bool isPrimary)
	{
		...
		::new (commandBuffers) VulkanCommandBuffer(pCommandBuffer, m_commandPool, m_device);
	...
	std::vector<RHICommandBuffer*> VulkanCommandPool::GetCommandBuffer(unsigned int count, bool isPrimary)
	{
		...
		for(unsigned int i = 0; i < count; ++i)
		{
			...
			::new (commandBuffers[i]) VulkanCommandBuffer(pCommandBuffer, m_commandPool, m_device);
		}
